...
### 1. Grayscale only
```bash
./imgtool images/uglydog.png output.png --grayscale
```
Converts the image to black and white using the luminance formula.

### 2. Brighten first, then grayscale
```bash
./imgtool images/uglydog.png output.png --brighten 60 --grayscale
```
...
```bash
./imgtool images/uglydog.png output.png --grayscale --brighten 60
```
...
```bash
./imgtool images/uglydog.png output.png --grayscale --brighten 30 --blur --flipH
```
...
```bash
./imgtool images/uglydog.png output.png --rotate 90 --flipV
```
